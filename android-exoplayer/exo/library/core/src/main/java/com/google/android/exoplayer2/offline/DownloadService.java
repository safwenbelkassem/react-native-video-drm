...
import static com.google.android.exoplayer2.offline.DownloadManager.paused;
...
public abstract class DownloadService extends Service {
  ...
  public static final String LINKS_SIZE = "LINKS_SIZE";
  ...
  public static Intent buildAddDownloadIntent(
      ...
      boolean foreground
  ,int linksize) {
    return buildAddDownloadIntent(context, clazz, downloadRequest, STOP_REASON_NONE, foreground,linksize);
  }
  ...
  public static void sendAddDownload(
      ...
      boolean foreground,
      int linkSize) {
    Intent intent = buildAddDownloadIntent(context, clazz, downloadRequest, foreground,linkSize);
  ...
  public static void sendAddDownload(
      ...
      int stopReason,
      boolean foreground ) {
    Intent intent = buildAddDownloadIntent(context, clazz, downloadRequest, stopReason, foreground,0);
    startService(context, intent, foreground);
  }
  ...
  public static void sendRemoveAllDownloads(
    ...
    android.util.Log.d(TAG, "sendRemoveAllDownloads: canceling downloads");
    startService(context, intent, foreground);
  }
    ...
    switch (intentAction) {
      ...
      case ACTION_ADD_DOWNLOAD:
        ...
        if (downloadRequest == null) {
          ...
          int linkSize = intent.getIntExtra(LINKS_SIZE, 0);

          downloadManager.addDownload(downloadRequest, stopReason,linkSize);
        }
        break;
      ...
      case ACTION_SET_STOP_REASON:
        if (!Assertions.checkNotNull(intent).hasExtra(KEY_STOP_REASON)) {
          ...
          android.util.Log.d(TAG, "sendRemoveAllDownloads: canceling all downloads from download service ");
  ...
  private static final class DownloadManagerHelper implements DownloadManager.Listener {
    ...
    public void onWaitingForRequirementsChanged(
      ...
      android.util.Log.d(TAG, "onWaitingForRequirementsChanged: .getDownloadsPaused : "+!downloadManager.getDownloadsPaused());
      android.util.Log.d(TAG, "onWaitingForRequirementsChanged:  serviceMayNeedRestart : "+serviceMayNeedRestart());
      android.util.Log.d(TAG, "onWaitingForRequirementsChanged:  waitingForRequirements: "+!waitingForRequirements);
      if (!waitingForRequirements
        ...
        android.util.Log.d(TAG, "Resuming paused downloads ");
