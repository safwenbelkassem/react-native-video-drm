...
public final class DownloadManager {
  ...
  public interface Listener {
    ...
    default void onDownloadsSucess(
            DownloadManager downloadManager,boolean sucess) {}
    /**
  ...
  public static final int DEFAULT_MAX_PARALLEL_DOWNLOADS =1;
  /** The default minimum number of times a download must be retried before failing. */
  public static final int DEFAULT_MIN_RETRY_COUNT = 3;
  ...
  private static final int MSG_DOWNLOAD_DISPATCH = 3;
  private static final int MSG_DOWNLOAD_FAIL_DISPATCH = 4;
...
private static int linkSize;
  private final Context context;
  private final WritableDownloadIndex downloadIndex;
  private static  Handler applicationHandler;
  private static InternalHandler internalHandler = null;
  private RequirementsWatcher.Listener requirementsListener;
  private DownloaderFactory downloaderFactory;
  private final CopyOnWriteArraySet<Listener> listeners;
  static HashMap<String,Downloader> downloaderHashMap = new HashMap<>();
  private static int pendingMessages;
  ...
  private static int notMetRequirements;
  private static boolean waitingForRequirements;
...
this.downloaderFactory = downloaderFactory;
  ...
  private int getDownloadValue(String id) {
  ...
  public void setStopReason(@Nullable String id, int stopReason) {
    android.util.Log.d(TAG, "setStopReason:  downloads ==="+downloads);
    android.util.Log.d(TAG, "setStopReason:  downloaderHashMap ==="+downloaderHashMap);
    ...
    //  downloadIndex.setDownloadingStatesToQueued();
      ...
      for (Download download :downloads
        ...
        android.util.Log.d(TAG, "setStopReason:  My downloads on cancel ==="+download.request.uri);

      }

  }
    ArrayList<String> list = new ArrayList<String>(downloaderHashMap.keySet());

    for (int i = 0; i < list.size(); i++) {
      Downloader downloader = downloaderHashMap.get(list.get(i));
    ...
    ArrayList<Download> counterDownloadlist = downloads;

    for (int i = 0; i < counterDownloadlist.size(); i++) {
      Download download = downloads.get(i);
     // if (download.state == STATE_DOWNLOADING || download.state == STATE_QUEUED) {
        if(list.contains(downloads.get(i).request.uri.toString())){
        //     int removeIndex = findDownloadIndex(download.request.id);
        int removeIndex = getDownloadValue(download.request.id);
  ...
  private int findDownloadIndex(String id) {
    for (int i = 0; i < downloads.size(); i++) {
      Download download = downloads.get(i);
      if (download.request.id.equals(id)) {
        return i;
  ...
  private void addDownload(DownloadRequest request, int stopReasonNone) {
   ...
   */
  public void
  addDownload(DownloadRequest request, int stopReason,int ls) {
    if(linkSize==0){
      linkSize = ls;
    }

    android.util.Log.d(TAG, "handleMessage: request for add =="+request.uri.toString());
    pendingMessages++;
    internalHandler
  ...
  private void setDownloadsPaused(boolean downloadsPaused) {
    android.util.Log.d(TAG, "Pausing downloads, are downloads paused ?? " +downloadsPaused);
  ...
  private void onRequirementsStateChanged(
    ...
    if (this.notMetRequirements != notMetRequirements) {
      ...
      android.util.Log.d(TAG, "onRequirementsStateChanged: called in download manager ");
  ...
  private boolean handleMainMessage(Message message) {
    switch (message.what) {
      case MSG_INITIALIZED:
        ArrayList<Download> downloads = (ArrayList<Download>) message.obj;
        onInitialized(downloads);
        break;
      case MSG_DOWNLOAD_UPDATE:
        int DownloadedCount = message.arg2;
      ...
      case MSG_DOWNLOAD_FAIL_DISPATCH:
        for (Listener listener : listeners) {
          listener.onDownloadsSucess(this,false);
        }


        break;

      case MSG_DOWNLOAD_DISPATCH:
        
        
        for (Listener listener : listeners) {
          listener.onDownloadsSucess(this,true);
        }
  ...
  private void onInitialized(ArrayList<Download> downloads) {
    initialized = true;
  ...
  private void onDownloadUpdate(DownloadUpdate update) {
    ...
    if (update.isRemove) {
      android.util.Log.d(TAG, "onDownloadUpdate: DELETING ALL OF THE DOWNLOADS ");
      for (Listener listener : listeners) {
    ...
    public void handleMessage(Message message) {
      ...
      switch (message.what) {
        ...
        case MSG_SET_DOWNLOADS_PAUSED:
          ...
          android.util.Log.d(TAG, "handleMessage:  Paused Downloads "+downloadsPaused);
...
if(!canceled){
    ...
    private void setNotMetRequirements(@Requirements.RequirementFlags int notMetRequirements) {
      android.util.Log.d(TAG, "setNotMetRequirements: called in download manager "+downloadsPaused);
//      if(!downloadsPaused){
//
//        DownloadCursor cursor = null;
//        try {
...
//      }
    ...
    private void setStopReason(@Nullable String id, int stopReason) {
      android.util.Log.d(TAG, "setStopReason: STOPPING EVERYTHING " + stopReason);
      if (id == null) {
        ArrayList<String> list = new ArrayList<String>(downloaderHashMap.keySet());
        for (int i = 0; i < downloads.size(); i++) {
          if(list.contains(downloads.get(i).request.uri.toString())){
            putDownloadWithState(downloads.get(i), STATE_REMOVING);
          }
        }
      // syncTasks();

//        List<Download> counterDownloadlist = downloads;
//        for (int i = 0; i < counterDownloadlist.size(); i++) {
//          Download download = downloads.get(i);
//          if (download.state == STATE_DOWNLOADING || download.state == STATE_QUEUED) {
//            //     int removeIndex = findDownloadIndex(download.request.id);
...
//      android.util.Log.d(TAG, "addDownload: Adding : "+download.request.uri.toString());
      if (download != null) {
    ...
    private void removeAllDownloads() {
      ...
      android.util.Log.d(TAG, "removeAllDownloads: ");
      successcount = 0;
      linkSize = 0;
      List<Download> terminalDownloads = new ArrayList<>();
      try (DownloadCursor cursor = downloadIndex.getDownloads(STATE_DOWNLOADING, STATE_QUEUED)) {
        while (cursor.moveToNext()) {
      ...
      for (int i = 0; i < downloads.size(); i++) {
       ...
       // mainHandler.obtainMessage(MSG_TASK_STOPPED).sendToTarget();
    ...
    private void syncTasks() {
      ...
      android.util.Log.d(TAG, "syncTasks:  downloads.size === "+downloads.size());
      for (int i = 0; i < downloads.size(); i++) {
        Download download = downloads.get(i);
        android.util.Log.d(TAG, "syncTasks: state : "+download.state);
        @Nullable Task activeTask = activeTasks.get(download.request.id);
    ...
    private void syncStoppedDownload(@Nullable Task activeTask) {
      android.util.Log.d(TAG, "syncStoppedDownload: ");
      if (activeTask != null) {
    ...
    private Task syncQueuedDownload(@Nullable Task activeTask, Download download){
      ...
      android.util.Log.d(TAG, "syncQueuedDownload: Syncing download for "+download.request.uri.toString());
      downloaderHashMap.put(download.request.uri.toString(),downloader);
      if(!canceled){
      // We can start a download task.
      download = putDownloadWithState(download, STATE_DOWNLOADING);
        android.util.Log.d(TAG, "syncQueuedDownload:  paused ?  === "+canDownloadsRun());
if(canDownloadsRun()){
    ...
    private void syncDownloadingDownload(
      ...
      android.util.Log.d(TAG, "syncDownloadingDownload:  Setting downloads to queue");
      Assertions.checkState(!activeTask.isRemove);
    ...
    private void syncRemovingDownload(@Nullable Task activeTask, Download download) {
      ...
      android.util.Log.d(TAG, "syncRemovingDownload: Starting a remove task for "+download.request.uri.toString());
    ...
    //  internalHandler.obtainMessage(MSG_REMOVE_DOWNLOAD, download.request.id).sendToTarget();
    }
    ...
    private void onTaskStopped(Task task) {
      ...
      android.util.Log.d(TAG, "onTaskStopped: ");
...
if(download != null){
      ...
      android.util.Log.d(TAG, "onTaskStopped: state of the task is "+download.state);
      android.util.Log.d(TAG, "onTaskStopped: is Removing ? "+isRemove);

      if(canceled || isRemove){
...
//      }
    ...
    private void onDownloadTaskStopped(Download download, @Nullable Exception finalException) {
      ...
      android.util.Log.d(TAG, "onDownloadTaskStopped: download.state  ==="+download.state );
      if(canDownloadsRun()){
...
if( download.state ==STATE_DOWNLOADING){
 ...
 failcount++;
      ...
      android.util.Log.d(TAG, "onDownloadTaskStopped:  Downloaded : "+successcount);
      android.util.Log.d(TAG, "onDownloadTaskStopped:  linkSize : "+linkSize);
      if(linkSize>0){

      if(successcount == linkSize ){
        android.util.Log.d(TAG, "onDownloadTaskStopped:  sucess !! ");
        mainHandler.obtainMessage(MSG_DOWNLOAD_DISPATCH).sendToTarget();
        successcount = 0;
        failcount = 0;
        linkSize = 0;
        downloads.clear();
        downloaderHashMap.clear();
      }else if (failcount+successcount == linkSize){
        mainHandler.obtainMessage(MSG_DOWNLOAD_FAIL_DISPATCH).sendToTarget();
        successcount = 0;
        failcount = 0;
        linkSize = 0;
        downloads.clear();
     ...
     // downloads.remove(getDownloadIndex(download.request.id));
      ...
      mainHandler.obtainMessage(MSG_DOWNLOAD_UPDATE, update).sendToTarget();
        ...
        for(int i=0;i<downloads.size();i++){
          DownloadUpdate update =
                  new DownloadUpdate(
                          downloads.get(i), /* isRemove= */ true, new ArrayList<>(downloads), finalException);
          mainHandler.obtainMessage(MSG_DOWNLOAD_UPDATE, update).sendToTarget();
        }
...
//        syncTasks();
//      } else {
        android.util.Log.d(TAG, "onRemoveTaskStopped: Calling removing");
   ...
   //   }
    ...
    private void updateProgress() {
      android.util.Log.d(TAG, "updateProgress: ");
      for (int i = 0; i < downloads.size(); i++) {
        Download download = downloads.get(i);
        if (download.state == STATE_DOWNLOADING) {
          try {
  ...
  private static class Task extends Thread implements Downloader.ProgressListener {
    ...
    public void cancel(boolean released) {
      ...
      if (!isCanceled) {
        android.util.Log.d(TAG, "cancel: Canceling");
        isCanceled = true;
        if(downloader!=null){
    ...
    @Override
    public void run() {
      android.util.Log.d(TAG, "run: Updating in run time ");
      try {
        if (isRemove) {
          android.util.Log.d(TAG, "run: Canceling downloaders ");
          if(downloader!=null){
            downloader.cancel();
            downloader.remove();
          }
