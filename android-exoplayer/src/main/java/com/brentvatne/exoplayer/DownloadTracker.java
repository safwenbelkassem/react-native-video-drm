...
import static com.brentvatne.exoplayer.ReactExoplayerView.dispatchFailEventtoRN;
import static com.brentvatne.exoplayer.ReactExoplayerView.dispatchSuccessEventtoRN;
import static com.brentvatne.exoplayer.ReactExoplayerView.getBytes;
import static com.brentvatne.exoplayer.ReactExoplayerView.linksSize;
import static com.brentvatne.exoplayer.ReactExoplayerView.mediaItems;
import static com.brentvatne.exoplayer.ReactExoplayerView.originallinks;
import static com.brentvatne.exoplayer.ReactExoplayerView.removeBytes;
import static com.brentvatne.exoplayer.ReactExoplayerView.setAccounts;
import static com.brentvatne.exoplayer.ReactExoplayerView.setBytes;

import static com.brentvatne.exoplayer.ReactExoplayerView.setDownloadState;
import static com.google.android.exoplayer2.offline.DownloadService.KEY_STOP_REASON;
...
public class DownloadTracker implements  DownloadHelper.Callback{
  ...
  public void onPrepared(DownloadHelper helper) {
    android.util.Log.d(TAG, "onPrepared:  jawha behi");
    helperPrepared();
  }

  @Override
  public void onPrepareError(DownloadHelper helper, IOException e) {
    android.util.Log.d(TAG, "onPrepareError: Error");
    dispatchFailEventtoRN();    dispatchFailEventtoRN();
  }

  public void setcanceledfalse() {
    DownloadManager.canceled = false;
  }
  ...
  public interface Listener {
    ...
    void onDownloadRemoved(Download state);

    void  onProgressChanged(float bytes);

    void  onErrorFetchLicence();
  }
  ...
  public static boolean fetchingKeys = false;
  private final HashMap<Uri, Download> downloads;
  ReactExoplayerView reactExoplayerView;
  static HashMap<String,DownloadRequest> DownloadLinks =new HashMap<>();
  static int Downloadedassets =0;
  static int removingCounter;
  ...
  public boolean isDownloaded(MediaItem mediaItem) {
    android.util.Log.d(TAG, "isDownloaded: Checking if downloaded : "+mediaItem.playbackProperties.uri);
    Download download = downloads.get(checkNotNull(mediaItem.playbackProperties).uri);
    return download != null && download.state != Download.STATE_FAILED && download.state != Download.STATE_STOPPED;
  }
  ...
  public void cancelDownloads() {
    // DownloadService.sendPauseDownloads(context, PlayerDownloadService.class,false);
    //   DownloadService.sendRemoveAllDownloads(context, PlayerDownloadService.class,false);
    //  removeBytes();
    // for (String urii:originallinks) {
    // refreshDeletedDownloads(Uri.parse(urii));

    //}
    // loadDownloads();
    DownloadService.sendSetStopReason(context, PlayerDownloadService.class,null,10,false);
  }

  private boolean  updateAssetwithKeySetId(MediaItem mediaItem,byte[] KeysetId) {
  ...
  public void toggleDownload(
          FragmentManager fragmentManager, MediaItem mediaItem, RenderersFactory renderersFactory,ReactExoplayerView rExoplayerView) {
    //  Download download = downloads.get(checkNotNull(mediaItem.playbackProperties).uri);
...
//              context, PlayerDownloadService.class, download.request.id, /* foreground= */ false);
//    } else {
    this.reactExoplayerView = rExoplayerView;
  ...
  private final class WidevineOfflineLicenseRenew extends AsyncTask<Void, Void, byte[]> {
    ...
    protected byte[] doInBackground(Void... voids) {
      ...
      try {
        ...
        android.util.Log.d(TAG, "doInBackground: Eror ");
    ...
    protected void onPostExecute(byte[] aVoid) {
     ...
     /*   MediaItem.Builder builder = mediaItem.build().buildUpon();
        ...
        android.util.Log.d(TAG, "onPostExecute: problem "+keySetId.length);
  ...
  private class DownloadManagerListener implements DownloadManager.Listener {
    ...
    @Override
    public void   onDownloadsSucess(  @NonNull DownloadManager downloadManager,boolean success){
      android.util.Log.d(TAG, "onDownloadsSucess: ");
      if(success){
    ...
    public void onDownloadRemoved(
      ...
      android.util.Log.d(TAG, "onDownloadRemoved: DELETING IN DOWNLOAD TRACKERS ");
      downloads.remove(download.request.uri);
      removeBytes(download.request.uri.toString());
      android.util.Log.d(TAG, "onDownloadRemoved:  Removing : "+download.request.uri);
removingCounter++;
if(removingCounter==linksSize){
    dispatchFailEventtoRN();
  removingCounter=0;
  android.util.Log.d(TAG, "onDownloadRemoved: Sending fail event to RN ");
}
      for (Listener listener : listeners) {
        listener.onDownloadsChanged(download);
      }
  ...
  private final class StartDownloadDialogHelper
    ...
    public StartDownloadDialogHelper(
            FragmentManager fragmentManager, DownloadHelper downloadHelper, MediaItem mediaItem, ReactExoplayerView reactExoplayerView) {
...
//      startDownload(downloadRequest);
    ...
    private void startDownload() {
      if(mediaItem!=null) {
        if(!DownloadManager.canceled){

   //     byte[]data =   getBytes(  mediaItem.playbackProperties.uri.toString());

      //    if (data==null) {
            setBytes(mediaItem.playbackProperties.uri.toString(), keySetId);
            android.util.Log.d(TAG, "startDownload: The asset is not downloaded, starting download now");
            DownloadRequest downloadRequest =  buildDownloadRequest(mediaItem.playbackProperties.uri.toString());

            if(downloadRequest!=null){

              DownloadLinks.put(mediaItem.playbackProperties.uri.toString(), downloadRequest  );
              if(DownloadLinks.size() == linksSize){
fetchingKeys= false;
                Iterator it = DownloadLinks.entrySet().iterator();
                setDownloadState(0);
                while (it.hasNext()) {
                  Map.Entry<String,DownloadRequest> pair = (Map.Entry)it.next();
                  System.out.println(pair.getKey() + " = " + pair.getValue());
                  startDownload(pair.getValue());
                  it.remove(); // avoids a ConcurrentModificationException
                }
//          for(int i=0;i<DownloadLinks.size();i++){
//            DownloadRequest DR =DownloadLinks.get;
//            android.util.Log.d(TAG, "startDownload:downloadRequest "+DR);
//            android.util.Log.d(TAG, "startDownload:downloadReques : Download links t "+DownloadLinks);
//            startDownload(DR);
//          }
                DownloadLinks.clear();
                Downloadedassets=0;
              ...
              android.util.Log.d(TAG, "startDownload: error getting downloadrequest");
              dispatchFailEventtoRN();
              DownloadLinks.clear();
            }
//          }else{
//            Downloadedassets++;
//            if(Downloadedassets==linksSize){
//              Downloadedassets=0;
//              dispatchSuccessEventtoRN();
//            }
          }
          ...
          android.util.Log.d(TAG, "startDownload: The item is downloaded");
          // dispatchSuccessEventtoRN();
        }
        //  mediaItems.remove(0);
        reactExoplayerView.setDownload();
      //}
    ...
    private void startDownload(DownloadRequest downloadRequest) {
      // android.util.Log.d(TAG, "handleMessage:  sendAddDownload =="+downloadRequest.uri.toString());

      DownloadService.sendAddDownload(
              context, PlayerDownloadService.class, downloadRequest, /* foreground= */ false,linksSize);
    }
    ...
    private  DownloadRequest buildDownloadRequest(String uri) {
      DownloadRequest downloadRequest = downloadHelper.getDownloadRequest(Util.getUtf8Bytes(checkNotNull(uri))).copyWithKeySetId(keySetId);
      android.util.Log.d(TAG, "handleMessage:  sendAddDownload =="+downloadRequest.uri.toString());
  ...
  private final class WidevineOfflineLicenseFetchTask extends AsyncTask<Void, Void, Void> {
    ...
    protected Void doInBackground(Void... voids) {
      ...
      try {
        ...
        drmSessionException = e;
        android.util.Log.d(TAG, "doInBackground:  Error");
        dispatchFailEventtoRN();
      }
//     //   offlineLicenseHelper.release();
//      }
