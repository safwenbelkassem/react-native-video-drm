...
public class ReactExoplayerViewManager extends ViewGroupManager<ReactExoplayerView> {
    ...
    private static final String PROP_CANCEL_DOWNLOAD = "canceldownload";
    private static final String PROP_DELETE_BOOK = "deleteBook";
    ...
    private static final String PROP_SRC_ACCOUNT_ID = "accountId";
    private static final String PROP_DRM = "drm";
    private static final String PROP_GET_ALL_DOWNLOADS = "getdownloads";
    private static final String PROP_GET_DOWNLOAD_STATE= "downloadstate";
    ...
    public void onDropViewInstance(ReactExoplayerView view) {
        Log.d(TAG, "onDropViewInstance:  hidden ===");
    ...
    @ReactProp(name = PROP_GET_DOWNLOAD_STATE)
    public void getDownloadState(final ReactExoplayerView videoView,String data) throws JSONException {
        videoView.getDownloadState();
    }
    ...
    public void setSrc(final ReactExoplayerView videoView, @Nullable ReadableMap src) {
        ...
        String accountid = src.hasKey(PROP_SRC_ACCOUNT_ID) ? src.getString(PROP_SRC_ACCOUNT_ID) : null;
        ...
        Log.d(TAG, "setSrc: accountid == "+accountid);
        if (startsWithValidScheme(uriString)) {
            Uri srcUri = Uri.parse(uriString);

            if (srcUri != null) {
                videoView.setSrc(srcUri, extension, headers,String.valueOf(accountid));
            }
        } else {
    ...
    @ReactProp(name = PROP_CANCEL_DOWNLOAD)
    public void cancelDownloads(final ReactExoplayerView videoView,boolean someval){
        Log.d(TAG, "cancelDownloads: Cancel called");
        videoView.cancelDownloads();

    }

    @ReactProp(name = PROP_DOWNLOAD)
    public void startDownload(final ReactExoplayerView videoView,ReadableMap download) {

        ReadableArray link = download.hasKey("link") ? download.getArray("link") : null;
       // Log.d( "startDownload:  link",link);
          String AccountId = download.hasKey("AccountId") ? download.getString("AccountId") : null;
        Log.d( "startDownload:  AccountId",AccountId);
       // String headers = download.hasKey(PROP_SRC_HEADERS) ? String(download.getMap(PROP_SRC_HEADERS)) : null;

        if(link!=null){

            ArrayList<String> list = new ArrayList();
            for(int i=0;i<link.size();i++){
                list.add(link.getString(i));
            }

             Log.d(TAG, "startDownload: ");
            videoView.setLinks(list,AccountId);
                videoView.setDownload();
            }else{
    ...
    @ReactProp(name = PROP_DELETE_BOOK)
    public void deleteBook(final ReactExoplayerView videoView, @Nullable ReadableMap deleteBook) {

        ReadableArray link = deleteBook.hasKey("link") ? deleteBook.getArray("link") : null;
        Log.d( "startDownload:  link", String.valueOf(link));
        String AccountId = deleteBook.hasKey("AccountId") ? deleteBook.getString("AccountId") : null;
        Log.d( "startDownload:  AccountId",AccountId);
        // String headers = download.hasKey(PROP_SRC_HEADERS) ? String(download.getMap(PROP_SRC_HEADERS)) : null;
        ...
        if(link!=null){
            ...
            videoView.deleteDownload(list,AccountId);
        }else{
            Log.d(TAG, "stopDownload: ");
        }
