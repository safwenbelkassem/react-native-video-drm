...
export default class Video extends Component {
  ...
  download = async(link,accontId) => {
    if (Platform.OS === 'android') {
      console.log("accontId ==== ",accontId);
      console.log("links ==== ",link);
      this.setNativeProps({

        download: {
          link: link,
          AccountId: accontId.toString()
        
        }
        }
         );
    }else{
      return await NativeModules.VideoManager.save(link,accontId, findNodeHandle(this._root));
    }
  };
  ...
  downloadstate = () => {
    console.log("Checking if download state is different  than 0");
    if (Platform.OS === 'android') {
      this.setNativeProps({ downloadstate: "" });
    }
  };
  ...
  deleteBook = async (link,accountId) => {
    if (Platform.OS === 'android') {
 
      console.log("delete with ",link,accountId)
      this.setNativeProps({
    ...
    return await NativeModules.VideoManager.deleteBook(link,accountId, findNodeHandle(this._root));
  }
  };


  CancelCurrentDownloads = async () => {
    console.log("Cancel all all current downloads")
    
    if (Platform.OS === 'android') {
      this.setNativeProps({ canceldownload: false });
    }else{
      return await NativeModules.VideoManager.CancelCurrentDownloads(findNodeHandle(this._root));
  ...
  _getAllDownloads = (event) => {
    if (this.props.getAllDownloads) {
      this.props.getAllDownloads(event.nativeEvent);
    }
  };
  ...
  _DownloadState = (event) => {
    if (this.props.DownloadState) {
      this.props.DownloadState(event.nativeEvent);
    }
  };

  _DownloadProgress = (event) => {
    if (this.props.DownloadProgress) {
      this.props.DownloadProgress(event.nativeEvent);
    }
  };
  ...
  render() {
    ...
    Object.assign(nativeProps, {
      ...
      getAllDownloads: this._getAllDownloads,
...
Video.propTypes = {
  ...
  getAllDownloads: PropTypes.func,
