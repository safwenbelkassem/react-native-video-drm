...
- (void)deleteBook:(NSArray *)assetLink resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
...
- (void)save:(NSArray*)link resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
    ...
    AllDownloadLinksCount = link.count;
    ...
    NSLog(@"we have %lu chapters in this book",(unsigned long)downloadLinks.count);
    int downloadcount = downloadLinks.count;
    
    for(id chapter in link){
        [self playableChapterAssetVerification:chapter link:link];
    }
...
- (void)URLSession:(NSURLSession *)session assetDownloadTask:(AVAssetDownloadTask *)assetDownloadTask didFinishDownloadingToURL:(NSURL *)location   API_AVAILABLE(ios(9.0)){
    ...
    NSLog(@"downloaded %d",downloadedTask);
    if(downloadedTask==downloadLinks.count){
        
        [_eventDispatcher sendAppEventWithName:@"onDownloadEnd" body: [NSNumber numberWithInt:AllDownloadLinksCount]];
    }
...
- (void)playableChapterAssetVerification:(NSString *)chapter link:(NSArray *)link {
        ...
        if (savedValue) {
            ...
            if (assetURL.isFileURL) {   // local
                ...
                if(downloadLinks.count == 0){
                    ...
                    [_eventDispatcher sendAppEventWithName:@"onDownloadEnd" body: [NSNumber numberWithInt:link.count]];
                }
