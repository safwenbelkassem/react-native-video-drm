        ...
        rejecter:(RCTPromiseRejectBlock)reject)
{
    ...
    [self.bridge.uiManager prependUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RCTVideo *> *viewRegistry) {
        ...
        if (![view isKindOfClass:[RCTVideo class]]) {
            ...
            [view save:link AccountID:AccountID resolve:resolve reject:reject];
        }
    }];
};
...
RCT_REMAP_METHOD(CancelCurrentDownloads,
       
        reactTag:(nonnull NSNumber *)reactTag
        resolver:(RCTPromiseResolveBlock)resolve
        rejecter:(RCTPromiseRejectBlock)reject)
{
    NSLog(@"CANCELING ALL DOWNLOADS IN NATIVE");
    [self.bridge.uiManager prependUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RCTVideo *> *viewRegistry) {
        RCTVideo *view = viewRegistry[reactTag];
        if (![view isKindOfClass:[RCTVideo class]]) {
            RCTLogError(@"Invalid view returned from registry, expecting RCTVideo, got: %@", view);
        } else {
            [view CancelCurrentDownloads];
        }
    }];
};


RCT_REMAP_METHOD(deleteBook,
        assetLink:(NSArray *)assetLink
        accountID:(NSInteger *)accountID
        reactTag:(nonnull NSNumber *)reactTag
        resolver:(RCTPromiseResolveBlock)resolve
        rejecter:(RCTPromiseRejectBlock)reject)
{
    NSLog(@"DELETE IN NATIVE  %@",assetLink);
    [self.bridge.uiManager prependUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RCTVideo *> *viewRegistry) {
        RCTVideo *view = viewRegistry[reactTag];
        if (![view isKindOfClass:[RCTVideo class]]) {
            RCTLogError(@"Invalid view returned from registry, expecting RCTVideo, got: %@", view);
        } else {
            [view deleteBook:assetLink accountID:accountID];
        }
    }];
};
